...
def pair_cr_term(cr_term_locations):
    ...
    # Keep the pairs of cr and termination sequences if # of nucleotides between them is <900 bp
    opa_locs = pd.concat([opas_forward, opas_back]).reset_index(drop = True)
    opa_locs = opa_locs[opa_locs['approx_orf_length']<=900].reset_index(drop = True)
...
def find_missing_cr(opa_metadata, seq_record_dict):
    ...
    if len(opa_metadata_missing_cr)>0:
        ...
        for i, row in opa_metadata_missing_cr.iterrows():
            ...
            cr_locations = []
            if row['strand']==1:
                approximate_orf = str(seq[int(row['start_term']-900):int(row['stop_term'])])
                matches = regex.finditer(r"(((CTCTT){e<=1}){3,100})", approximate_orf)
                for m in matches:
                    cr_locations.append([m.start()+row['start_term']-900, m.end()+row['start_term']-900])
                ...
                if len(cr_locations)==0:
                    ...
                    if match is not None:
                        cr_locations.append([match.end()+row['start_term']-900, match.end()+row['start_term']-900])
            else:
                approximate_orf = str(seq[int(row['start_term']):int(row['stop_term']+900)])
...
def get_opa_locs(opa_metadata, seq_record_dict, strain):
    ...
    for i, row in opa_metadata.iterrows():
        ...
        # For now, I only know how to find the ORFs if both the CRs and termination sequence have been found, but I'm naming all opa genes (to catch opa genes where there were no repeats; however, this will also catch instances where there's only a very short opa gene fragment that is non-functional). Currently working on.
        ...
        if ~np.isnan(row['start_cr']):
            ...
            opa_metadata.loc[i,'start']=np.nan
            opa_metadata.loc[i,'stop']=np.nan
...
def opa_seqs_to_fasta(opa_metadata, seq_record_dict, sample_name):
    ...
    SeqIO.write(records, 'opa_sequences/'+sample_name+".fa", "fasta")

def opa_seqs_no_cr(opa_metadata, seq_record_dict, sample_name):
    records = []
    for i, row in opa_metadata.iterrows():
        if (~np.isnan(row['start']))&(~np.isnan(row['stop'])):
            seq = seq_record_dict[row['chromosome']].seq
    ...
    SeqIO.write(records, 'opa_sequences_no_repeats/'+sample_name+'.fa', 'fasta')
    
def save_metadata(opa_metadata, sample_name):
    """Save metadata file"""
    opa_metadata.to_csv('opa_locations/' + sample_name + '.csv')
...
def main():    
    ...
    opa_seqs_to_fasta(opa_metadata, seq_record_dict, sample_name)
    opa_seqs_no_cr(opa_metadata, seq_record_dict, sample_name)
    save_metadata(opa_metadata, sample_name)
