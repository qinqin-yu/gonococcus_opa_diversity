        ...
        for line in infile:
            if line[0] != "#":
                ...
                if POS == "2603":
                    ...
                    freqs_2050 = INFO.split(";")[6].split("=")[1].split(",")
        return {"2603":freqs_2603, "2051":freqs_2051, "2050":freqs_2050}
    ...
    with open(f"itol_{variant}.txt", "w") as itolfile:
        ...
        for s in allele_dict:
            ...
            if allele_dict[s][pos] == "C" and variant == "16S_C1192T":
                ...
                color = colors[1]
            elif allele_dict[s][pos] == "A" and variant == "16S_G478T":
                color = colors[2]
            else:
                print(f"{variant} has new alternate allele ({allele_dict[s][pos]} in {s}), please edit script")
                sys.exit(0)
            itolfile.write(f"{s}\t{color}\t{str(allele_dict[s][pos])}\n")

def write_summary_table(A2058G, A2059G, C2611T, alleles_16S):
    with open("rRNA_allele_summary.txt", "w") as outfile:
        outfile.write("wgs_id\t23S_A2058G\t23S_A2059G\t23S_C2611T\t16S_C1192T\t16S_478\n")
        for s in A2059G:
            pos_1192_16S = alleles_16S[s]["1192"]
            pos_478_16S = alleles_16S[s]["478"]
            outfile.write(f"{s}\t{A2058G[s]}\t{A2059G[s]}\t{C2611T[s]}\t{pos_1192_16S}\t{pos_478_16S}\n")
...
copy_number_dict_A2058G = call_copy_number(dict_23S_freqs, "2050")
copy_number_dict_A2059G = call_copy_number(dict_23S_freqs, "2051")
copy_number_dict_C2611T = call_copy_number(dict_23S_freqs, "2603")
write_itol_23S(copy_number_dict_A2058G, ["#ffffff", "#f2f0f7", "#cbc9e2", "#9e9ac8", "#6a51a3"], "23S_A2058G")
...
write_itol_16S(dict_16S_alleles, ["#fff7bc", "#fec44f", "#d95f0e"], "16S_G478T")
write_summary_table(copy_number_dict_A2058G, copy_number_dict_A2059G, copy_number_dict_C2611T, dict_16S_alleles)
