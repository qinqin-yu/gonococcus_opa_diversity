...
def get_args():
    ...
    parser.add_argument("fig_filename", help="filename for figure of opa assignment definitions")
...
plt.hist(opa_metadata['start_reordered_flipped'], 200, density = True, alpha = 0.8, label = '$\it{N. gonorrhoeae}$')
plt.yscale('log')
...
plt.text(0.04*10**6, 10**-4.7, 'A,B')
plt.text(0.8*10**6, 10**-4.7, 'C,D,E')
plt.text(1.2*10**6, 10**-4.7, 'F')
plt.text(1.4*10**6, 10**-4.7, 'G,H,I')
plt.text(1.8*10**6, 10**-4.7, 'J')
plt.text(2.0*10**6, 10**-4.7, 'K')

plt.ylim(ylim)
plt.xlabel('Genome position')
plt.ylabel('Density of opa genes')
plt.legend()
plt.tight_layout()
plt.savefig(args.fig_filename, dpi = 300)
plt.show()